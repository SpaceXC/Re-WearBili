...
class Media3PlayerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        BilibiliSdkManager.initSdk(
            dataManager = DataStoreManager(application)
        )
        ...
        val webiSignatureKey = intent.getStringExtra(PARAM_WEBI_SIGNATURE_KEY).logd("sign")
        viewModel.playVideoFromId(videoIdType!!, videoId!!, videoCid, webiSignatureKey)
        setContent {
            Media3PlayerScreen(
                viewModel = viewModel,
