...
class UserSpaceDynamicPagingSource(
    ...
    override suspend fun load(params: LoadParams<Pair<Int, String?>>): LoadResult<Pair<Int, String?>, DynamicItem> {
        try {
            ...
            val url =
                "https://api.bilibili.com/x/polymer/web-dynamic/v1/feed/space?host_mid=$mid&timezone_offset=-480&type=all${if (currentPage == 1 || dynamicIdOffset.isNullOrEmpty()) "" else "&offset=$dynamicIdOffset"}&page=$currentPage".logd(
                    "dynamicUrl"
                )!!
